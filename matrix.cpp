...
std::vector<std::vector<double>> vec_main_diameter_of_double_matrix ( std::vector<std::vector<double>> wises ) {

}

std::vector<std::vector<double>> vec_side_diameter_of_double_matrix ( std::vector<std::vector<double>> wises ) {

  
}

double matrixs_detarmination_in_rule_of_sarrus ( std::vector<double> matrixs_wise , std::vector<std::vector<double>> lines_and_columns_of_the_matrix ) {
  std::vector<std::vector<double>> double_matrix ( 2 );  
  int counter_for_matrixs_wise {};

  for ( int counter { 0 }; counter < double_matrix.size(); ++ counter ) {
    
    for ( int nested_counter { 0 }; nested_counter < lines_and_columns_of_the_matrix[ 0 ][ 0 ] * lines_and_columns_of_the_matrix[ 0 ][ 1 ]; ++ nested_counter ) {
      double_matrix[ counter ].push_back( matrixs_wise[ counter_for_matrixs_wise ] );   
      ++ counter_for_matrixs_wise;
    }
  ...
  for ( int counter { 0 }; counter < double_matrix.size(); ++ counter ) {
    ...
    for ( int nested_counter { 0 }; nested_counter < lines_and_columns_of_the_matrix[ 0 ][ 0 ] * lines_and_columns_of_the_matrix[ 0 ][ 1 ]; ++ nested_counter ) {
      std::cout << double_matrix[ counter ][ nested_counter ] << '\t';
    }
...
double matrix_detrmination_operation( std::vector<std::vector<double> > square_matrixs_lines_and_columns , std::vector<double> matrixs_wise ) {
  ...
  if ( square_matrixs_lines_and_columns[ 0 ][ 0 ] == 1 ) {
    ...
    if ( square_matrixs_lines_and_columns[ 0 ][ 0 ] == 2 ) {
      ...
      matrixs_detarmination_in_rule_of_sarrus( matrixs_wise , square_matrixs_lines_and_columns );
