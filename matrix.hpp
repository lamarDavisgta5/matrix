...
# include <vector>  
  ...
  std::vector<std::vector<double>> matrixs_1_inputs( std::vector<std::vector<double>> ); 
  int matrix_determination( std::vector<std::vector<double> > , std::vector<double> ); 
  std::string quit_or_continue( std::vector<std::vector<double>> , int , int , int , int );
# endif
